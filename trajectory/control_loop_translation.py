...
def listener():
    rospy.init_node('control_loop_translation')
    
    # Run listener nodes, with the option of happeneing simultaneously.
    rospy.Subscriber('trajectory_cl_o', trajectory, callback_trajectory)
    rospy.Subscriber('ninedof_vals', ninedof, callback_trajectory)

    global Publisher
    Publisher = rospy.Publisher('trajectory_cl_l', trajectory, queue_size=10)
